...
function handleClick(sender, args)
{
	if (args.rawEventArgs.button == 0)
	{
		...
		var mtype = (<HTMLSelectElement>document.getElementById("markerType")).value;
		if (mtype == "marker-question")
			mark.imageSrc = "../assets/question.png";
		if (mtype == "marker-house")
			mark.imageSrc = "../assets/icon_home.png";
		mark.cssClass = mtype;
		view.decorations.add(mark);

		var text = (<HTMLInputElement>document.getElementById("bubbleText")).value;
		// create a new bubble decoration
		if (text != "")
		{
			var bubble = new m.Bubble(args.location);
			bubble.text = (<HTMLInputElement>document.getElementById("bubbleText")).value;
