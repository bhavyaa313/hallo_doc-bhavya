...
function loadMap(position)
{
	...
	{
		var center = dataList[(e.target as HTMLSelectElement).value].location;
		view.panTo(center);
	}
	...
	l.urlTemplate = "http://tiles.stadiamaps.com/tiles/stamen_terrain/{z}/{x}/{y}.png";
...
{
	// monitor map data
	(<HTMLInputElement>document.getElementById("centerLat")).value = sender.mapCenter.latitude;
	(<HTMLInputElement>document.getElementById("centerLong")).value = sender.mapCenter.longitude;

	(<HTMLInputElement>document.getElementById("currentLat")).value = args.location.latitude;
	(<HTMLInputElement>document.getElementById("currentLong")).value = args.location.longitude;

	(<HTMLInputElement>document.getElementById("currentX")).value = args.position.x;
	(<HTMLInputElement>document.getElementById("currentY")).value = args.position.y;

	(<HTMLInputElement>document.getElementById("scrollX")).value = sender.scrollX;
	(<HTMLInputElement>document.getElementById("scrollY")).value = sender.scrollY;
}
