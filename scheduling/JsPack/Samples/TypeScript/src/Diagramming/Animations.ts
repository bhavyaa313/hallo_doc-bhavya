...
namespace AnimationsSample {
    ...
    const DiagramEvents = Diagramming.Events;
    const Animation = Animations.Animation;
    const AnimationEvents = Animations.Events;
    const Point = Drawing.Point;
    const Rect = Drawing.Rect;
    const GlassEffect = Diagramming.GlassEffect;
    var PulseAnimation = Diagramming.Animations.PulseAnimation;
    var ScrollAnimation = Diagramming.Animations.ScrollAnimation;

    
    let diagram: Diagramming.Diagram;
    let startNode: Diagramming.ShapeNode,
        endNode: Diagramming.ShapeNode,
        branchOne: Diagramming.ShapeNode,
        branchTwo: Diagramming.ShapeNode;
    let pulseAnimation: Animations.Animation;
    let nodeAnimations = new Array<Animations.Animation>(),
        linkAnimations = new Array<Animations.Animation>();
            ...
            nodeAnimations.push(new Animation(node,
                {
                    ...
                    animationType: +(<HTMLSelectElement>document.getElementById('animationType')).value,
                    easingType: +(<HTMLSelectElement>document.getElementById('easingType')).value
                }, onUpdateNode));
        });
    }
    ...
    function onAnimationTypeChanged() {
        ...
        nodeAnimations.forEach(function (a) {
            let aType = +(document.getElementById('animationType') as HTMLSelectElement).value;
    ...
    function onEasingTypeChanged() {
        ...
        nodeAnimations.forEach(function (a) {
            if (!a.isRunning)
                a.easingType = (+(document.getElementById('easingType') as HTMLSelectElement).value);
        });
    }
    ...
    function createLink(origin, destination, labelText?) {
        let link = new DiagramLink(diagram, origin, destination);
        link.visible = false;
    ...
    function createPulseAnimation(item) {
        ...
        let a = new PulseAnimation(options);
        item.animate(a);
        return a;
    }
    ...
    function animateScroll(toValue) {
        ...
        let a = new ScrollAnimation(
            {
                fromValue: new Point(diagramView.scrollX, diagramView.scrollY),
                toValue: new Point(diagramView.scrollX, toValue),
                duration: 1500,
                easingType: Animations.EasingType.EaseOut,
                animationType: Animations.AnimationType.Power,
                param: 3
            });
        diagramView.animate(a);
    }
